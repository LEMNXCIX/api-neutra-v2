...
const mockUserRepository: IUserRepository = {
    ...
    create: async () => ({ success: false }) as never,
    update: async () => ({ success: false }) as never,
    linkProvider: async () => ({ success: false }) as never,
    getUsersStats: async () => [],
...
async function verify() {
    ...
    console.log(
        "Initial Config:",
        JSON.stringify(createResult.data.config, null, 2),
    );
    ...
    console.log(
        "Updated Config:",
        JSON.stringify(updateResult.data.config, null, 2),
    );
    ...
    if (
        updateResult.data.config?.branding?.tenantLogo !==
    ...
    if (
        updateResult.data.config?.settings?.supportEmail !== "test@example.com"
    ) {
